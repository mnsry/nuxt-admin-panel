<template>
  <v-main>
    <v-container fluid>
      ...
      <v-row v-else>
        <product-pc-edit v-if="!mobile" />

        <product-mobile-edit v-if="mobile" />
      </v-row>
    </v-container>
  </v-main>
</template>
...
export default {
  ...
  mounted() {
    this.$store.commit('navbar/updateNav', 'محصولات | ویرایش محصول جدید')
  },
  head: {
    titleTemplate: '%s | محصولات: ویرایش اطلاعات',
  },
  fetchOnServer: false
}
</script>
