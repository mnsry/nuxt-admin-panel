  ...
  <v-container fluid>
    ...
    <v-row>
      <v-col cols="12">
        ...
        <v-btn :to="{ name: 'product-id-edit' }" color="success" text outlined>
          ویرایش محصول
        </v-btn>
      </v-col>
    </v-row>
      ...
      <v-col cols="12">
        <v-card color="rgba(0,0,0,0.3)" rounded>
          <v-card-text>
            ...
            <br />
            <span>توضیحات :</span>
            <p class="text--primary">{{ product.body }}</p>
            <br />
            <span>نام کاربر سازنده :</span>
            <p>{{ user.name }}</p>
            <br />
            <span>عکس:</span>
          ...
          <v-card-actions>
            <v-btn color="red" outlined @click="deleteProduct">حذف شود</v-btn>
          </v-card-actions>
        </v-card>
      </v-col>
    </v-row>
